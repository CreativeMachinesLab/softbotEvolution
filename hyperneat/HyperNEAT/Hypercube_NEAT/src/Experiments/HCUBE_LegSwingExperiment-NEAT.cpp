...
namespace HCUBE
{
	...
	LegSwingNEATExperiment::LegSwingNEATExperiment(string _experimentName)
	:
	...
	Hybrid_FTMutateLinkProbability(-1.0)
	{
		...
		if(int(NEAT::Globals::getSingleton()->getParameterValue("LimitLegSwings")) == 1) {
		...
		if(_experimentName == "HYBRID") {
			cerr<<"Hybrid is not yet implemented\n";
			exit(-1);
	...
	GeneticPopulation* LegSwingNEATExperiment::createInitialPopulation(int populationSize)
	{
...
//		int numSheets;
//#ifdef HIDDEN_LAYER  // NEAT starts simple, no HIDDEN_LAYER
//		numSheets = 3;
//#else
		numSheets = 2;
//#endif
...
//#endif
	...
	NEAT::GeneticPopulation* LegSwingNEATExperiment::createInitialPopulation(shared_ptr<NEAT::GeneticPopulation> CPPN_pop, shared_ptr<Experiment> HyperNEAT_experiment) {
	...
	void LegSwingNEATExperiment::generateSubstrate()
	{
//		int numSheets;
//#ifdef HIDDEN_LAYER
//		numSheets = 3;
...
//#endif
	...
	void LegSwingNEATExperiment::populateSubstrate(shared_ptr<const NEAT::GeneticIndividual> individual)
	{
		substrate = individual->spawnFastPhenotypeStack<double>(); //genome = phenotype for neat
	}
	
	double LegSwingNEATExperiment::processEvaluation(shared_ptr<NEAT::GeneticIndividual> individual, bool postHocRecord = false)
	{
	...
	void LegSwingNEATExperiment::processGroup(shared_ptr<NEAT::GeneticGeneration> generation)
	{
	...
	void LegSwingNEATExperiment::processIndividualPostHoc(shared_ptr<NEAT::GeneticIndividual> individual)
	{
	...
	void LegSwingNEATExperiment::printSubstrate(shared_ptr<const NEAT::GeneticIndividual> individual)
	{
	...
	void LegSwingNEATExperiment::printActualSubstrate()
	{
		cout << "Printing substrate" << endl;
		ofstream substrate_file;        
		substrate_file.open ("substrateThatSolvedTheProblem.txt", ios::trunc );
		...
		for (int counter=0;counter<numLinks;counter++)
		{
			...
			substrate_file << substrate.getLink(counter)->fromNode << " --> " << substrate.getLink(counter)->toNode << ": ";
	...
	void LegSwingNEATExperiment::printCPPN(shared_ptr<const NEAT::GeneticIndividual> individual)
	{
		assert(false);  // NEAT does not have CPPNs
...
/*		cout << "Printing cppn network" << endl;
	...
	Experiment* LegSwingNEATExperiment::clone()
	{
		cout << "Experiment cloned\n";
		LegSwingNEATExperiment* experiment = new LegSwingNEATExperiment(*this);
