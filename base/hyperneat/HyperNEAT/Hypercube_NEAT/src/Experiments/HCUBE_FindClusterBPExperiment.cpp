...
#define FIND_CLUSTER_BP_EXPERIMENT_DEBUG (0)

#define FIND_CLUSTER_BP_EXPERIMENT_USE_DELTAS (0)

namespace HCUBE
{
    using namespace NEAT;

    FindClusterBPExperiment::FindClusterBPExperiment(string _experimentName)
            :
    ...
    NEAT::GeneticPopulation* FindClusterBPExperiment::createInitialPopulation(int populationSize)
    {
...
#if FIND_CLUSTER_BP_EXPERIMENT_USE_DELTAS
        ...
        //genes.push_back(GeneticNodeGene("Sigmoid1","HiddenNode",0.5,false,ACTIVATION_FUNCTION_SIGMOID));
        //genes.push_back(GeneticNodeGene("Sigmoid2","HiddenNode",0.5,false,ACTIVATION_FUNCTION_SIGMOID));
        //genes.push_back(GeneticNodeGene("Sigmoid3","HiddenNode",0.5,false,ACTIVATION_FUNCTION_SIGMOID));
        //genes.push_back(GeneticNodeGene("Sigmoid4","HiddenNode",0.5,false,ACTIVATION_FUNCTION_SIGMOID));
        //genes.push_back(GeneticNodeGene("Sigmoid5","HiddenNode",0.5,false,ACTIVATION_FUNCTION_SIGMOID));
        //genes.push_back(GeneticNodeGene("Sigmoid6","HiddenNode",0.5,false,ACTIVATION_FUNCTION_SIGMOID));
        //genes.push_back(GeneticNodeGene("Sigmoid7","HiddenNode",0.5,false,ACTIVATION_FUNCTION_SIGMOID));
        //genes.push_back(GeneticNodeGene("Sigmoid8","HiddenNode",0.5,false,ACTIVATION_FUNCTION_SIGMOID));
        genes.push_back(GeneticNodeGene("Sin1","HiddenNode",0.5,false,ACTIVATION_FUNCTION_SIN));
        genes.push_back(GeneticNodeGene("Sin2","HiddenNode",0.5,false,ACTIVATION_FUNCTION_SIN));
        genes.push_back(GeneticNodeGene("Sin3","HiddenNode",0.5,false,ACTIVATION_FUNCTION_SIN));
        genes.push_back(GeneticNodeGene("Sin4","HiddenNode",0.5,false,ACTIVATION_FUNCTION_SIN));
        genes.push_back(GeneticNodeGene("Gaussian1","HiddenNode",0.5,false,ACTIVATION_FUNCTION_GAUSSIAN));
        genes.push_back(GeneticNodeGene("Gaussian2","HiddenNode",0.5,false,ACTIVATION_FUNCTION_GAUSSIAN));
        genes.push_back(GeneticNodeGene("Gaussian3","HiddenNode",0.5,false,ACTIVATION_FUNCTION_GAUSSIAN));
        genes.push_back(GeneticNodeGene("Gaussian4","HiddenNode",0.5,false,ACTIVATION_FUNCTION_GAUSSIAN));
    ...
    void FindClusterBPExperiment::generateSubstrate()
    {
    ...
    void FindClusterBPExperiment::populateSubstrate(shared_ptr<const NEAT::GeneticIndividual> individual)
    {
            ...
            for (int x1=0;x1<numNodesX;x1++)
            {
                for (int y2=0;y2<numNodesY;y2++)
                {
                    for (int x2=0;x2<numNodesX;x2++)
                    {
                        ...
                        CPPNNetwork.reinitialize();
                        CPPNNetwork.setValue("X1",x1Val);
                        CPPNNetwork.setValue("Y1",y1Val);
                        CPPNNetwork.setValue("X2",x2Val);
                        CPPNNetwork.setValue("Y2",y2Val);

                        if (CPPNNetwork.hasNode("DeltaX"))
                        {
                            CPPNNetwork.setValue("DeltaX",x2Val-x1Val);
                            CPPNNetwork.setValue("DeltaY",y2Val-y1Val);
                        }

                        CPPNNetwork.setValue("Bias",0.3);
                        CPPNNetwork.update();


                        double greyVal = CPPNNetwork.getValue("Output");
    ...
    {
#if FIND_CLUSTER_BP_EXPERIMENT_DEBUG
    ...
    void FindClusterBPExperiment::processGroup(shared_ptr<NEAT::GeneticGeneration> generation)
    {
        ...
        correctedNames.push_back("Output");

        cout << "Getting error values for CPPN...";
        int linkCount=0;
        ...
        for (int y1=0;y1<numNodesY;y1++)
        {
            for (int x1=0;x1<numNodesX;x1++)
            {
                for (int y2=0;y2<numNodesY;y2++)
                {
                    for (int x2=0;x2<numNodesX;x2++)
                    {
                        ...
                        CPPNNetwork.setValue("Bias",0.3);
                        CPPNNetwork.dummyActivation();
                        CPPNNetwork.update(2);
                        ...
                        */
                        ...
                        correctedValues.clear();
                        double newWeight = substrate.getLink(counter)->weight;

                        newWeight /= 3.0;
                        newWeight *= 0.8;

                        if(fabs(newWeight)<0.2)
                        {
                            newWeight = 0.0;
                        }
                        else if(newWeight>0.2)
                        {
                            newWeight -= 0.2;
                        }
                        else
                        ...
                        correctedValues.push_back(newWeight);
                        CPPNNetwork.backProp(correctedNames,correctedValues,false);

                        errorSum += fabs(newWeight - weight);
            ...
            cout << "TOTAL FITNESS: " << fitness << " / " << maxFitness << endl;
...
#ifndef HCUBE_NOGUI
    bool FindClusterBPExperiment::handleMousePress(wxMouseEvent& event,wxSize &bitmapSize)
    {
    ...
    void FindClusterBPExperiment::increaseResolution()
    {
        if (sizeMultiplier==1)
        {
    ...
    void FindClusterBPExperiment::decreaseResolution()
    {
        if (sizeMultiplier==3)
        {
            numNodesX /= 3;//((numNodesX-1)/2)+1;
    ...
    Experiment* FindClusterBPExperiment::clone()
    {
        FindClusterBPExperiment* experiment = new FindClusterBPExperiment(*this);
